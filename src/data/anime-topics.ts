...
split.words.forEach((w, i) => colors[i] = utils.get(w, 'color'));
            ...
            {
              id: 'nier-loading', label: '尼尔加载',
              desc: 'NieR 终端加载动画：纯 Canvas 2D 点阵渲染 + 终端逐字输出。65 行 YoRHa 徽章 ASCII 内嵌为字符串数组；DENSITY_MAP 字符→亮度映射（@ 最亮 . 最暗）；fillRect 逐点绘制，STRETCH_X/Y 控制宽高比；resize 自适应重绘；typeLine 逐字插入 span + 随机 glitch 乱码闪烁 + rAF 颜色渐变；点击重新播放（loop）。不依赖 anime.js，纯 rAF + Promise。',
              html: `<div id="nier-title"><span class="big">LOADING</span><span class="small">_系统启动中<span id="nier-dots"></span></span></div>
<canvas id="nier-grid-art"></canvas>
<div id="nier-lines"></div>`,
              css: `body { margin: 0; padding: 0; overflow: hidden; background: #1C1814; background-image: linear-gradient(rgba(0,0,0,0.15) 1px, transparent 1px), linear-gradient(90deg, rgba(0,0,0,0.15) 1px, transparent 1px); background-size: 5px 5px; font-family: 'Courier New', 'Source Code Pro', 'Consolas', 'Noto Sans SC', monospace; letter-spacing: 0.05em; }
#nier-grid-art { position: fixed; inset: 0; width: 100%; height: 100%; z-index: 5; pointer-events: none; }
#nier-title { position: fixed; top: 16px; left: 24px; z-index: 10; color: #fff; letter-spacing: 0.15em; }
#nier-title .big { font-size: 22px; }
#nier-title .small { font-size: 13px; margin-left: 6px; }
#nier-lines { position: fixed; top: 20%; left: 4%; z-index: 10; color: #fff; font-size: 12px; line-height: 1.9; }
#nier-lines p { white-space: nowrap; margin: 0; }`,
              js: `(function () {
  var canvas = document.getElementById('nier-grid-art');
  var linesEl = document.getElementById('nier-lines');
  ...
  function drawGridArt() {
    ...
    for (var r = 0; r < ASCII_EMBLEM.length; r++) {
      var row = ASCII_EMBLEM[r];
  ...
  async function typeLine(text, isLast) {
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      if (Math.random() < 0.08) {
        ...
        el.appendChild(gs); await sleep(28); el.removeChild(gs);
      }
      el.appendChild(span);
  ...
  animate(el, {
    innerHTML: scrambleText({ text: mottoes[mottoIdx] }),
        ...
        },
        {
          id: 'manda-draw',
          label: 'manda描边',
          desc: `**manda 描边·轨迹底稿(明暗双层)** — 完整线稿先以暗影浮现当「轨迹底」,再逐笔高亮描画。

同一组 11 元素线稿叠成两层:
- **.manda-ghost(轨迹底)** — 整组 opacity 淡入至 0.16 后常驻,暗紫轮廓始终可见
- **.manda-stroke(高亮层)** — \`createDrawable('.line')\` 逐笔 \`draw: '0 1'\`,画完的线保留叠加

循环节奏:底稿浮现 → 逐笔书写(400ms 错峰、1600ms/笔)→ 画完整幅停留 3s → 亮层淡出(只留底稿)→ 复位重画。`,
          html: `<div class="manda-wrap">
  <svg id="manda-svg" viewBox="0 0 304 112" fill="none" fill-rule="evenodd"
    stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    <path stroke-width="2" d="M 101.3031 39.3883 L 101.3031 83.0327 L 112.2881 83.0327 L 112.2881 39.353 L 101.3031 39.353" />
    <path stroke-width="2.0007459" d="M 134.4968 39.3883 L 134.4968 83.0327 L 145.4818 83.0327 L 145.4818 39.353 L 134.4968 39.353" />
</g>
<g class="manda-stroke" stroke="#D3C1F7" fill="none" fill-rule="evenodd"
       stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
    <path class="line" id="path1-1" stroke-width="1.28852" d="m 101.16998,83.139621 v -21.75251 c -0.21971,-6.202523 -5.024027,-11.007293 -10.985007,-11.007293 -6.066941,0 -10.984992,4.888918 -10.984992,10.919931 0,6.031015 4.918051,10.919929 10.984992,10.919929 h 5.492514 v 10.919943 h -5.492514 c -12.13389,0 -21.969986,-9.777839 -21.969986,-21.839872 0,-12.062028 9.836096,-21.839864 21.969986,-21.839864 12.025347,0 21.603617,9.631379 21.970007,21.759572 v 21.920164 z" />
    <path class="line" id="path1-6" stroke-width="1.28852" d="M 210.09927,83.033584 V 61.281073 c -0.21973,-6.202522 -5.02404,-11.007291 -10.98502,-11.007291 -6.06694,0 -10.985,4.888917 -10.985,10.919932 0,6.031016 4.91806,10.919928 10.985,10.919928 h 5.4925 v 10.919942 h -5.4925 c -12.13389,0 -21.96999,-9.777837 -21.96999,-21.83987 0,-12.062029 9.8361,-21.839864 21.96999,-21.839864 12.02533,0 21.60362,9.631378 21.97,21.759571 v 21.920163 z" />
    <path class="line" id="path1-6-2" stroke-width="1.289" d="M167.5065 82.7886 m 7.29 0 V 68.1586 c -0.24 -8.09 -6.6 -14.52 -14.57 -14.52 c -8.05 0 -14.57 6.52 -14.57 14.57 c 0 8.05 6.52 14.57 14.57 14.57 h 3.64 v -7.29 h -3.64 c -4.02 0 -7.29 -3.26 -7.29 -7.29 c 0 -4.02 3.26 -7.29 7.29 -7.29 c 3.95 0 7.14 3.21 7.29 7.34" />
    <path class="line" id="polyline1-1" stroke-width="2" d="M 210.0992 39.3755 L 210.0992 83.0327 L 221.0842 83.0327 L 221.0842 39.353 L 210.0992 39.353" />
    <path class="line" id="polyline1-1-3" stroke-width="2" d="M 165.6462 39.3748 L 165.6462 83.0321 L 176.6312 83.0321 L 176.6312 39.3523 L 165.6462 39.3523" />
    <path class="line" id="path3-9" stroke-width="1.28852" d="M 2.5723411,83.033584 V 61.026703 C 2.9387244,48.984588 12.516993,39.35385 24.542327,39.35385 c 12.133899,0 21.969998,9.777835 21.969998,21.839864 v 21.83987 H 35.527332 v -21.83987 c 0,-6.031015 -4.91805,-10.919932 -10.985005,-10.919932 -5.960971,0 -10.76529,4.804769 -10.984991,11.007291 v 21.752511 z" />
    <path class="line" id="path3-9-5" stroke-width="1.28852" d="M 35.446859,83.033584 V 61.026703 C 35.813241,48.984588 45.391512,39.35385 57.416845,39.35385 c 12.133901,0 21.96999,9.777835 21.96999,21.839864 v 21.83987 H 68.401853 v -21.83987 c 0,-6.031015 -4.918053,-10.919932 -10.985008,-10.919932 -5.960971,0 -10.76529,4.804769 -10.98499,11.007291 v 21.752511 z" />
    <path class="line" id="path3-9-5-1" stroke-width="1.28852" d="M 101.3023,83.033584 V 61.026703 c 0.36637,-12.042115 9.94464,-21.672853 21.96997,-21.672853 12.1339,0 21.97001,9.777835 21.97001,21.839864 v 21.83987 h -10.98501 v -21.83987 c 0,-6.031015 -4.91804,-10.919932 -10.985,-10.919932 -5.96097,0 -10.76528,4.804769 -10.98499,11.007291 v 21.752511 z" />
    <path class="line" id="path8-9" stroke-width="1.28852" d="m 226.52851,72.225105 a 5.4924973,5.4599662 0 1 1 -5.47763,5.824185 v -0.728426 c 0.19063,-2.839179 2.56338,-5.095759 5.47763,-5.095759 z" />
    <path class="line" id="polyline2-2" stroke-width="2" d="M 101.3031 39.3883 L 101.3031 83.0327 L 112.2881 83.0327 L 112.2881 39.353 L 101.3031 39.353" />
    <path class="line" id="polyline2-2-5" stroke-width="2.0007459" d="M 134.4968 39.3883 L 134.4968 83.0327 L 145.4818 83.0327 L 145.4818 39.353 L 134.4968 39.353" />
</g>
  </svg>
</div>`,
          css: `html, body { height: 100%; }
...
.manda-wrap { text-align: center; }
#manda-svg { width: min(400px, 92vw); height: auto; display: block; margin: 0 auto 18px; }
.manda-ghost { opacity: 0; } /* 轨迹底:初始隐藏,JS 淡入至 0.16 常驻 */
`,
          js: `import { animate, svg, stagger } from 'https://esm.sh/animejs@4.4.1';

const $ghost = document.querySelector('.manda-ghost');   // 轨迹底(暗影线稿)
const $stroke = document.querySelector('.manda-stroke'); // 高亮描画层
const items = svg.createDrawable('.line');               // 每条线一个可绘制目标
const wait = (ms) => new Promise((r) => setTimeout(r, ms));

// 节奏参数(想调快慢改这里)
const GAP  = 400;   // 笔与笔错峰延迟 ms
const DRAW = 1600;  // 单笔描画时长 ms
const HOLD = 3000;  // 画完整幅后的停留 ms(亮线保留时间)
const FADE = 1200;  // 亮层淡出时长 ms
const IDLE = 500;   // 消失后到下一轮的空档 ms

(async () => {
  // 首帧归零:线条先全部隐藏,避免加载瞬间闪现完整亮稿
  animate(items, { draw: '0 0', duration: 16 });
  await wait(80);

  // 轨迹底浮现并常驻(暗影版完整线稿)
  await animate($ghost, { opacity: [0, 0.16], duration: 1200, ease: 'outCubic' }).finished;

  // 明暗双层循环:逐笔高亮描画 → 全幅停留 → 亮层淡出(留底稿) → 复位重画
  for (;;) {
    // 逐笔画出:每笔错峰 GAP,画完的线保留叠加,直至整幅完成
    animate(items, { draw: '0 1', ease: 'inOutQuad', duration: DRAW, delay: stagger(GAP) });
    // 显式等待全部画完(总时长 = 单笔 + 最后一笔错峰),不依赖多目标 finished
    await wait(DRAW + (items.length - 1) * GAP + 80);
    // 画完整幅,亮线保留停留,看清整幅 manda
    await wait(HOLD);
    // 亮层整体淡出,只剩轨迹底
    await animate($stroke, { opacity: 0, duration: FADE, ease: 'inQuad' }).finished;
    // 复位:先隐藏全部线条(给足一帧),再恢复亮层透明度,准备下一轮书写
    animate(items, { draw: '0 0', duration: 16 });
    await wait(80);
