  ...
  animate(el, {
    innerHTML: scrambleText({ text: mottoes[mottoIdx] }),
        ...
        },
        {
          id: 'manda-draw',
          label: 'manda描边',
          desc: `**manda 逐笔描边** — 用 anime.js 官方 SVG drawable 把清理后的线稿逐条画出。

页面加载后循环:每条 .line 依次描画出来 → 短暂全显 → 整体消失 → 重新描画。

先做了几何清理(否则 dash/draw 类动画会卡顿、断线):
- 所有 \`transform\` 已烘焙进坐标,元素不再带 matrix 属性
- 画布外残留线条已剔除
- 弧线保持圆弧命令,半径与 sweep 按矩阵正确换算

绘制写法:
- \`svg.createDrawable('.line')\` 把每条线转成可绘制目标
- \`draw: ['0 0', '0 1', '1 1']\` 三个关键帧:空 → 整条画出 → 消失
- \`stagger(100)\' 逐条错峰,\`loop: true\` 无限循环`,
...
<g stroke="#D3C1F7" fill="none" fill-rule="evenodd"
       stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
...
// 逐笔绘制:每条 .line 依次画出,短暂全显后整体消失,循环
animate(svg.createDrawable('.line'), {
  draw: ['0 0', '0 1', '1 1'], // 空 → 全部画出 → 消失
  ease: 'inOutQuad',
  duration: 2000,
  delay: stagger(100),
  loop: true
